...
import { getFetchEventList, postFetchCreateEvent, postFetchDeleteEvent, postFetchUpdateEvent} from '../../services/calendarServices'
...
function* eventCreate(action: ReturnType<typeof createEvent>) {
  try{
    ...
    yield put({ type: SystemActionType.SHOW_TOAST, payload: { title: '알 수 없는 오류', content: '' } })
...
function* eventDelete(action: ReturnType<typeof deleteEvent>) {
  try{
    ...
    yield put({ type: CalendarActionTypes.SUCCESS_DELETE_EVENT, payload: res.data })

    if (action.meta){
      action.meta()
    }

    yield put({ type: SystemActionType.SHOW_TOAST, payload:{title:'삭제 완료', content:''}})
...
function* eventUpdate(action: ReturnType<typeof updateEvent>) {
  try{
    ...
    yield put({ type: CalendarActionTypes.SUCCESS_UPDATE_EVENT, payload: res.data })

    if (action.meta){
      action.meta()
    }

    yield put({ type: SystemActionType.SHOW_TOAST, payload:{title:'수정 완료', content:''}})
